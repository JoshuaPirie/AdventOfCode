            ...
            while((line = file.ReadLine()) != null) {
                ...
                for(int i = 0; i < matches[2].Captures.Count; i++) {
                    ...
                    foreach(KeyValuePair<string, int> result in results) {
                        if(result.Key == matches[2].Captures[i].Value) {
                            if(matches[2].Captures[i].Value == "cats" || matches[2].Captures[i].Value == "trees") {
                                if(int.Parse(matches[3].Captures[i].Value) > result.Value)
                                    containsCompound = true;
                            }
                            else if(matches[2].Captures[i].Value == "pomeranians" || matches[2].Captures[i].Value == "goldfish") {
                                if(int.Parse(matches[3].Captures[i].Value) < result.Value)
                                    containsCompound = true;
                            }
                            else if(int.Parse(matches[3].Captures[i].Value) == result.Value)
                                containsCompound = true;
                        }
                    }
                    if(!containsCompound)
